...
25. **Task - Markdown-only context menu fix** (v1.1.19): Restricted "Markdown Tools" submenu to only appear for markdown files and notebook markdown cells<br>
    **Result**: Two-layer fix in `src/index.ts`. First, extracted shared `isMarkdownContext()` helper that checks `app.shell.currentWidget` against tracker widgets before evaluating file type - previously the `isVisible` callbacks checked `editorTracker?.currentWidget` and `notebookTracker?.currentWidget` independently, so if a notebook was open anywhere the menu items appeared in non-markdown file editors (Python, JSON, etc.) because the notebook check passed. All 6 commands now reference `isMarkdownContext` instead of duplicating the check inline. Second, the submenu itself was still visible (even with all items hidden) because the context menu selector `.jp-FileEditor` matches all file editors - fixed by adding a `jp-MarkdownFileEditor` CSS class to markdown file editor widgets via `editorTracker.widgetAdded` and `editorTracker.forEach`, then changing the selector to `.jp-FileEditor.jp-MarkdownFileEditor`. Published v1.1.19 to npm and PyPI. CI lint check failed on `package-lock.json` formatting because `npm install` in `make build` regenerates it without Prettier formatting - fixed by running `npx prettier --write package-lock.json` post-build
